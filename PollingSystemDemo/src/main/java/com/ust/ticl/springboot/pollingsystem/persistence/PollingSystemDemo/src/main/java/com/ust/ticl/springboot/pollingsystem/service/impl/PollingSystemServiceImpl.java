	...
	public BaseResponse saveActualPolling(ActualPollingDetails actualPollingDetails) {
		...
		if (currentDate.after(pollingDetail.getFromDate())
			...
			if(Objects.nonNull(pollingUsers) && !pollingUsers.isEmpty()) {
				for(PollingUser pollingUser : pollingUsers) {
					if(userName.equalsIgnoreCase(pollingUser.getUserName())) {
						actual_pollingId.append(pollingUser.getUserId());
						List<ActualPolling> actualPollings = pollingSystemDomain.findAllActualPollings();
						boolean alreadyPolledUser = false;
						for(ActualPolling exsistingPolls : actualPollings) {
							if((exsistingPolls.getActualPollId() == Long.parseLong(actual_pollingId.toString()))) {
								alreadyPolledUser = true;
							}
						}
						if(FLAG_YES.equalsIgnoreCase(pollingUser.getPrivileged()) && !alreadyPolledUser) {
							actualPolling.setActualPollId(Long.parseLong(actual_pollingId.toString()));
							...
							actualPolling.setSelectedOption(actualPollingDetails.getSelectedOption());
							...
							baseResponse.setMessage("Sorry! The user has already polled");
							break;
						}
					}
				}
